...
bool runScript(char **commandLine, int pos, int max, int &returnValue, string &showPoly){
  ...
  if(! strcmp(commandLine[pos],"--with-args")){ 	
    ...
    string command2 = "opt  -load RangeAnalysis.so -load DepGraph.so -load ComplexityInference.so -mem2reg -loop-simplify \
    ...
    string command4 = "opt -load RangeAnalysis.so -load DepGraph.so -load ComplexityInference.so -mem2reg -correlated-propagation -instcombine \
...
bool runDefault(char **commandLine, int pos, int max, int &returnValue, string &showPoly){
  ...
  string command_ = "opt -load RangeAnalysis.so -load DepGraph.so -load ComplexityInference.so -instnamer -mem2reg -break-crit-edges -vssa \
